 ...
 */
// The ref lands on the input, which is what v2 handed to useMenuButton as
// well. Only the element type it was declared with was wrong.
      ...
      >
        ...
        <Menu.Trigger asChild>
          <Input
            ...
            css={{
              // v2's focusBorderColor is gone. v3's input recipe reads the ring
              // colour out of this custom property, and the `colors.` prefix is
              // not optional: a custom property's value goes through
              // tokens.getVar, which needs the category to find the token, so a
              // bare `brand.500` would reach the browser verbatim and the ring
              // would lose its colour.
              '--focus-color': `colors.${focusBorderColor}`
            }}
            {...styleProps?.parent}
            onKeyDown={e => {
              if (e.key === 'Escape') {
            ...
            onKeyDownCapture={e => {
              if (e.key === 'Escape') {
                ...
                e.key === 'Enter' &&
                menu.open &&
                // v2 asked whether the focus ring sat on no item at all.
                // v3 spells that as no highlighted value.
