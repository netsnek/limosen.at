...
const HamburgerMenuIcon: FC<IHamburgerMenuIconProps> = ({ handleClick, wrapperProps, iconProps }) => {
  ...
  return (
    <Box position="relative" rounded="full" boxSize="100%" onClick={handleClick} {...props}>
      <Box position="absolute" top="34%" left="25%" w="50%" h="4%" backgroundColor="limosen.border.subtle" borderRadius="full" {...iconProps} />
      <Box position="absolute" top="49%" left="25%" w="50%" h="4%" backgroundColor="limosen.border.subtle" borderRadius="full" {...iconProps} />
      <Box position="absolute" top="64%" left="25%" w="50%" h="4%" backgroundColor="limosen.border.subtle" borderRadius="full" {...iconProps} />
    </Box>
  );
}
...
export default function App() {
  ...
  return (
    <Box minH="100vh" bg="limosen.bg.canvas" color="limosen.text.primary" display="flex" flexDirection="column">
...
export function HeaderBar() {
  return (
    <Box bg="limosen.bg.banner" py={2} className="header-bar" display={{ base: 'none', md: 'block' }}>
      <Container maxW="6xl">
...
export function TopNavigation({ path }: { path?: string }) {
  ...
  return (
    <Box pos="relative" overflow="hidden" backgroundColor="limosen.bg.navTop" height={isOpen ? 'calc(100vh + 15px)' : { base: '12vh', md: '15vh' }} minH={isOpen ? '600px' : '100px'} transition="height 0.2s cubic-bezier(0.68, 0, 0.27, 1), min-height 0.2s cubic-bezier(0.68, 0, 0.27, 1)">
      <Box pos="relative" bg="limosen.bg.navTop" color="limosen.text.primary" transition="height 0.2s cubic-bezier(0.68, 0, 0.27, 1), min-height 0.2s cubic-bezier(0.68, 0, 0.27, 1)" height={isOpen ? 'max(600px, calc(100vh + 15px))' : '0'} minH={isOpen ? 'fit-content' : '0'} width="100%" overflow="hidden">
...
function AboutSection() {
  return (
    <Box as="section" bg="limosen.bg.about" py={{ base: 12, md: 20 }} className="about-us">
      <Container maxW="6xl">
...
function ServicesSection() {
  ...
  return (
    <Box as="section" bg="limosen.bg.section" py={{ base: 12, md: 20 }} id="services">
      <Container maxW="6xl">
                ...
                >
                  ...
                  <Box p={6}>
                    <LinkOverlay href={targetHref} display="block" onClick={(e) => handleServiceLinkClick(e, targetHref)}>
                      <Stack spacing={3}>
                        <Heading size="sm" color="limosen.text.primary">{service.title}</Heading>
                  ...
                  <AccordionItem key={service.id} id={service.id} border="none" mb={4}>
                    <h3>
                      <AccordionButton
                        ...
                        bg="whiteAlpha.50"
                        _expanded={{ bg: 'whiteAlpha.200', borderColor: 'limosen.accent', color: 'limosen.text.primary' }}
                        borderRadius="lg"
                        px={{ base: 4, md: 6 }}
                        py={{ base: 4, md: 5 }}
                        border="1px solid"
          ...
          <Accordion allowToggle reduceMotion>
            {FAQ_ITEMS.map((item) => (
              <AccordionItem key={item.question} border="none" mb={3}>
                ...
                <AccordionPanel px={{ base: 4, md: 6 }} pt={4} pb={6} color="limosen.text.secondary">{item.answer}</AccordionPanel>
              </AccordionItem>
            ))}
          </Accordion>
        </VStack>
...
function FleetSection() {
  return (
    <Box as="section" id="fahrzeuge" bg="limosen.bg.fleet" py={{ base: 12, md: 20 }}>
      <Container maxW="6xl">
          ...
          <SimpleGrid columns={{ base: 1, md: 2 }} spacing={{ base: 8, md: 10 }}>
            {FLEET_VEHICLES.map((vehicle) => (
              ...
              >
                ...
                <Box w="100%" h={{ base: '220px', md: '260px' }} overflow="hidden" bg="white">
                  <Field.Image
                ...
                <Stack spacing={3} p={6}>
                  <Heading size="md" color="limosen.text.primary">{vehicle.category}</Heading>
...
function KundenfeedbackSection() {
  return (
    <Box as="section" id="kundenfeedback" bg="limosen.bg.section" py={{ base: 12, md: 20 }}>
      <Container maxW="6xl">
        <VStack spacing={{ base: 8, md: 12 }} align="stretch">
          ...
          <Grid templateColumns={{ base: '1fr', lg: '1fr 1fr' }} gap={{ base: 6, md: 10 }}>
            ...
            <Box bg="limosen.bg.card" border="1px solid" borderColor="limosen.border.faint" borderRadius="xl" overflow="hidden" boxShadow="lg" minH={{ base: '280px', md: '360px' }}>
            ...
            <SimpleGrid columns={{ base: 1, sm: 2, md: 3 }} spacing={{ base: 8, md: 10 }} flex="1">
              {FOOTER_LINK_GROUPS.map((group) => (
                <VStack key={group.title} spacing={4} align="flex-start">
                  <Text fontWeight="bold" textTransform="uppercase" letterSpacing="widest" color="limosen.text.primary">{group.title}</Text>
                  <VStack spacing={2} align="flex-start">
                    {group.links.map((link) => (
                      <Link key={link.label} href={link.href} color="limosen.text.muted" _hover={{ color: 'limosen.text.primary' }}>
                        {link.label}
                      </Link>
                    ))}
                  </VStack>
