...
/**
 * UPDATED: useServiceAccordionNavigation
 * - Keeps refs to AccordionButtons
 * - Scrolls after layout with double rAF
 * - Applies index mapping & hash/custom event handling
 */
function useServiceAccordionNavigation(serviceIds: string[]) {
  ...
  // Refs to the AccordionButtons for precise scrolling
  ...
  const scrollToService = useCallback((id: string) => {
    ...
    // Ensure the layout has settled: double requestAnimationFrame
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener(SERVICE_NAVIGATION_EVENT, handleNavigation as EventListener);
  }, [openServiceById]);
      ...
      <Container maxW="6xl">
        ...
        >
          <HStack spacing={3} className="header-bar__item">
            ...
            <Link href={`mailto:${CONTACT_EMAIL}`} color="#ffffffff">
              {CONTACT_EMAIL}
            </Link>
          </HStack>
          <HStack spacing={3} className="header-bar__item">
              ...
              {SERVICES_CONTENT.map((service) => {
                ...
                return (
                  ...
                  >
                    <h3>
                      <AccordionButton
                        // UPDATED: attach ref for precise scrolling + offset for sticky header
                        ref={(el) => {
                      ...
                      >
                        ...
                        <Stack spacing={4} color="whiteAlpha.900" fontSize="md" flex="1">
                          {blocks.map((block, blockIndex) => {
                            ...
                            return <Text key={blockIndex}>{(block as any).text}</Text>;
                          })}
                        </Stack>
                      </Stack>
                    </AccordionPanel>
